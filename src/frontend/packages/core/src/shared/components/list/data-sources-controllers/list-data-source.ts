...
export abstract class ListDataSource<T, A = T> extends DataSource<T> implements IListDataSource<T> {
  ...
  public updateFlattenMax() {
    ...
    // TODO: RC dispatch action that will set everything back to default and is maxed ignored = true
    ...
    // this.masterAction.flattenPaginationMax = null;
    // if (Array.isArray(this.action)) {
    //   this.action.forEach(iAction => iAction.flattenPaginationMax = null);
    // } else {
    //   this.action.flattenPaginationMax = null;
    // }
    // this.store.dispatch(new ResetPagination(this.masterAction, this.masterAction.paginationKey));
